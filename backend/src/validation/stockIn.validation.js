...
exports.createStockInSchema = Joi.object({
    ...
    productType: Joi.string().valid("rebar", "non-rebar").required(),
    tons: Joi.number().positive().required(),
    currency: Joi.string().valid("UZS", "USD").required(),
    usdRate: Joi.number().positive().optional(),
    pieces: Joi.number().positive().optional(),
    pieceLengthM: Joi.number().positive().optional(),
    pricePerTon: Joi.number().positive().required(),
}).custom((value, helpers) => {

    if (value.productType === "non-rebar") {
        if (!value.pieces || !value.pieceLengthM) {
            return helpers.message(
                "non-rebar uchun pieces va pieceLengthM kiritilishi shart"
            );
        }
    }

    if (value.currency === "USD" && !value.usdRate) {
        return helpers.message("USD bo'lsa usdRate kerak");
    }
